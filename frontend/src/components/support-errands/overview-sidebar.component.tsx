...
export const OverviewSidebar: FC<OverviewSidebarProps> = ({ selectedView, onSelectView, open, onOpenChange }) => (
  ...
  >
    <nav className="flex flex-1 flex-col gap-16" aria-label="Ärenden">
      {open ?
        <h2 className="m-0 text-small font-bold text-dark-secondary">Ärenden</h2>
      : null}
      <ul className="m-0 p-0 list-none flex flex-col gap-8">
        {ERRAND_VIEWS.map((view) => {
          const Icon = VIEW_ICON[view.key];
          ...
          return (
            <li key={view.key}>
              <button
                ...
                aria-label={open ? undefined : view.label}
