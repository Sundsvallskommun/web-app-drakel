...
/**
 * Create/edit form for a mall or frastext. The category is fixed by the button the handläggare came in
 * through, so only the name, the type it belongs to and the text itself are editable.
 *
...
export const TemplateEditorModal: FC<{
  ...
  onClose: () => void;
  onSaved: () => void;
}> = ({ category, types, template, onClose, onSaved }) => {
  ...
  const canSave = name.trim() !== '' && code !== '' && !saving;
      ...
      <Modal.Content className="flex flex-col gap-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <FormControl id="template-name" className="w-full">
            <FormLabel>{t('editor.name')}</FormLabel>
            <Input
              value={name}
              onChange={(event) => {
