...
const main = async () => {
  ...
  await exec(
    `npx swagger-typescript-api generate --modular -p ${PATH_TO_OUTPUT_DIR}/backend/swagger.json -o ${PATH_TO_OUTPUT_DIR}/backend --no-client --clean-output --extract-enums`,
    callback
  );
};
